"""Fire & smoke detector — Module 10.

Wraps a YOLO model (``ai/weights/fire-smoke.pt``) to detect fire and
smoke in camera frames.  When YOLO or the weight file is missing the
...
class FireSmokeDetector(BaseDetector):
    """Fire and smoke detection backed by a YOLOv8 model."""
    ...
    def available(self) -> bool:
        """True when YOLO + the weight file are both present."""
    ...
    def detect(self, frame: Frame) -> list[DetectionResult]:
        ...
        for result in results:
            for box in getattr(result, "boxes", []):
                ...
                incident_type = (
                    IncidentType.SMOKE if "smoke" in label.lower()
